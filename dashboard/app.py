...
def fetch_indicators_at_time(symbol: str, as_of_time: str, timeframe: str = "daily") -> dict:
    ...
    # Fetch 5-min intraday data (up to ~60 days) + daily for longer SMAs
...
st.sidebar.markdown("### 🕒 Time-of-Day Replay")
...
if _auto_refresh_active:
    ...
    if _alert_key != _prev_alert:
        if 4 <= _notify_quality <= 7 and _cascade.explosion_score >= 4:
        ...
        elif _notify_quality >= 8:
            send_browser_notification(
                title=f"🔔 {_notify_symbol} HIGH QUALITY — {_notify_direction} {_notify_quality}/11",
                body=(
                    f"${_notify_price:.2f} | Quality {_notify_quality}/11 | "
                    f"Cascade {_cascade.explosion_score}/10 | "
                    f"OR: {_or_direction} ({_or_momentum:+d}) | Recent: {_recent_dir} ({_recent_momentum:+d})"
                ),
                urgency="normal",
            )
            st.session_state["_last_alert_key"] = _alert_key
...
_cascade_color = "#e74c3c" if _cascade.explosion_score >= 7 else "#f39c12" if _cascade.explosion_score >= 4 else "#95a5a6"
_cascade_border = "3px solid" if _cascade.explosion_score >= 7 else "2px solid" if _cascade.explosion_score >= 4 else "1px solid"
_cascade_bg = "linear-gradient(135deg, #1a0a0a, #2d1010)" if _cascade.explosion_score >= 4 else "linear-gradient(135deg, #0d0d0d, #1a1a2e)"
_otm_html = f'<div style="font-size: 13px; color: #f0c040; margin-top: 6px;">💡 Consider <b>{_cascade.recommended_strike_offset} strike(s) OTM</b> for higher leverage (cheaper premium, 5x–10x potential)</div>' if _cascade.recommended_strike_offset > 0 else ''
_cascade_flags = (
    f"{'🔥 Momentum ACCELERATING &nbsp;' if _cascade.acceleration_detected else ''}"
    f"{'💀 Volume CLIMAX &nbsp;' if _cascade.volume_climax else ''}"
    f"{'⛓️ Multi-level CASCADE &nbsp;' if _cascade.cascade_breakdown else ''}"
)
if not _cascade_flags.strip():
    _cascade_flags = "No explosive signals detected — normal conditions"
_data_badge = f' <span style="font-size: 10px; color: #666; margin-left: 8px;">({_cascade.data_source})</span>'
st.markdown(
f"""<div style="background: {_cascade_bg}; border-radius: 12px;
padding: 16px; border: {_cascade_border} {_cascade_color}; margin: 8px 0;">
<div style="font-size: 18px; font-weight: 800; color: {_cascade_color};">
{_cascade.urgency} Explosion Potential: {_cascade.explosion_score}/10{_data_badge}
</div>
<div style="font-size: 13px; color: #d4d4d4; margin-top: 6px;">
{_cascade_flags}
</div>
{_otm_html}
</div>""",
...
# Backtest-validated: Q 4–6 has the best risk/reward (not too early, not too late)
...
_sweet_spot_active = _sweet_spot_quality and _sweet_spot_cascade
_sweet_spot_prime = _sweet_spot_quality and _cascade.explosion_score >= 7

if _sweet_spot_prime:
    _ss_color = "#00ff88"
    _ss_border = "3px solid"
    _ss_bg = "linear-gradient(135deg, #0a1a0a, #102d10)"
    _ss_icon = "🎯💰"
    _ss_label = "PRIME SWEET SPOT"
    _ss_desc = (
        f"Quality {_best_q}/13 (optimal range) + Explosion {_cascade.explosion_score}/10 — "
        f"<b>Highest probability entry.</b> Backtest shows Q 4–6 with cascade ≥ 7 produces the best R:R. "
        f"You're catching the move early with explosive momentum confirming."
    )
elif _sweet_spot_active:
    _ss_color = "#40e0d0"
    _ss_border = "2px solid"
    _ss_bg = "linear-gradient(135deg, #0a0d1a, #10152d)"
    _ss_icon = "🎯"
    _ss_label = "SWEET SPOT"
    _ss_desc = (
        f"Quality {_best_q}/13 (optimal range) + Explosion {_cascade.explosion_score}/10 — "
        f"Good entry zone. Not chasing (Q < 8) but confirmed enough (Q ≥ 4). "
        f"Monitor for cascade acceleration to size up."
    )
elif _sweet_spot_quality:
    _ss_color = "#6cb4ee"
    _ss_border = "1px solid"
    _ss_bg = "linear-gradient(135deg, #0d0d15, #151525)"
    _ss_icon = "🎯"
    _ss_label = "QUALITY SWEET SPOT"
    _ss_desc = (
        f"Quality {_best_q}/13 is in the optimal zone (4–7). "
        f"Explosion only {_cascade.explosion_score}/10 — wait for cascade signals before entering."
    )
else:
    _ss_color = None
...
if _ss_color:
    ...
    st.markdown(
f"""<div style="background: {_ss_bg}; border-radius: 12px;
padding: 14px; border: {_ss_border} {_ss_color}; margin: 8px 0;">
<div style="font-size: 16px; font-weight: 800; color: {_ss_color};">
{_ss_icon} {_ss_label} — {_ss_direction}
</div>
<div style="font-size: 13px; color: #d4d4d4; margin-top: 6px;">
{_ss_desc}
</div>
<div style="font-size: 11px; color: #888; margin-top: 4px;">
📊 Backtest insight: Q ≥ 8 is negative EV (chasing). Q 4–6 with cascade activation = best avg P&L.
</div>
</div>""",
        unsafe_allow_html=True,
    )
