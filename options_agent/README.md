...
> **OOS-90 staleness note (2026-06-06):** Numbers above predate the FOMC sit-out (2026-06-04) and failed-bounce sit-out (2026-06-06) promotions. They reflect the **older** golden config and the OOS window has slid 3 days forward. A fresh OOS-90 audit is needed next session before the new goldens can be claimed not-overfit; until then, walk-forward generalization for the failed-bounce promotion is best read from the 365d numbers above (which DO use current goldens and show LARGER Sharpe lifts than 730d).
>
...
**Exit-cohort separation** (730d, SPY, current goldens 2026-06-06):
- `decay_target` (win exit): 500 trades (57% of book) — the dominant win pathway
- `stagnation`: 209 trades (24%) — documented structural drag (see [stagnation memos](../.claude/projects/c--Users-krish-options-algo-trader/memory))
- `stop`: 145 trades (16%) — clean separation between win-state and loss-state; failed-bounce filter cut ~6 stop-out trades
- `time_stop`: 3 trades (<1%)
...
>
> **2026-06-06 promotion — Failed-bounce day-regime sit-out (`--skip-failed-bounce`, default ON):** Skip days where today's open gaps **up small** (0% < gap ≤ +0.50%) AND yesterday's close sat in the bottom 20% of yesterday's range. Mechanism: prior-day bearish capitulation close + weak overnight bid = the rally fails as underlying bearish flow returns. Diagnostic source: 1D scans of day-start features (gap %, prior-day range %, prior-day close position, VIX level/change, day-of-week) on golden-config triggers were ALL positive — existing gates (FOMC, gap-up, vix_max) had already filtered the obvious losers. A 2D `gap_pct × prior_day_close_pos` scan surfaced the cross-symbol negative cell: SPY 19 days mean −0.16R / total −2.99R; QQQ 10 days mean −0.33R / total −3.25R. **A/B clean sweep all 8 cells under real-options pricing:** SPY 730d PF 1.84→2.00 / Sharpe 3.12→**3.45** / Calmar 17.02→18.41 / MDD% 5.7→5.3 / PnL +$25; QQQ 730d PF 1.41→1.46 / Sharpe 1.81→1.99 / Calmar 5.52→5.99 / MDD% 18.1→**16.7** / PnL +$18; SPY 365d Sharpe 3.15→**3.64** / Calmar 10.06→11.29 / MDD% 9.4→8.5 / PnL +$17; QQQ 365d PF 1.29→1.36 / Sharpe 1.52→**1.84** / MDD% 37.7→36.3 / PnL +$13. Sharpe lifts are *larger* on 365d than 730d for both symbols — the cleanest possible walk-forward generalization sign. **Methodology lesson** from this A/B: a first pass ran on `--no-real-options` (synth pricing) because the trainer-extraction scripts used it; synth A/B showed agreed direction but overstated magnitudes (QQQ 365d Calmar synth lift +9.13 → real +0.10). Memory: [feedback_real_options_ab_discipline](../.claude/projects/c--Users-krish-options-algo-trader/memory/feedback_real_options_ab_discipline.md). The promotion stood under real options. **Significance:** second day-start filter to promote (after FOMC); confirms calendar/regime features are the open frontier after six consecutive entry-feature-diagnostic failures. Live agent: filter ported into `run_sweet_spot_agent.py` reading prior-day OHLC from Alpaca daily bars. Disable with `--allow-failed-bounce`; tune via `--failed-bounce-gap-max` (default 0.50) / `--failed-bounce-close-pos-max` (default 0.20).
>
> **2026-06-04 promotion — FOMC sit-out (`--skip-fomc`, default ON):** Skip the 8/year FOMC rate-decision days entirely. Diagnostic source: bucketed 730d triggers by macro-calendar flags (FOMC / CPI / PPI / NFP) and found FOMC days are the only cohort to clear both *elevated stop-rate* AND *negative aggregate P&L* on both symbols simultaneously — SPY 53.8% stop-rate vs 18.7% baseline / −$10.17 over 9 days; QQQ 31.0% vs 22.7% / −$9.42 over 11 days. Mechanism is plausible: the 2:00 PM ET presser creates a "frozen positioning" morning where institutional flow is muted, weakening the OR-breakout signal that the strategy's edge depends on. **A/B clean sweep all 4 cells:** SPY 730d PF 1.78→1.84 / Sharpe 3.02→**3.14** / Calmar 16.58→17.15 / MDD% 5.8→5.7 / PnL +$1,017; QQQ 730d PF 1.37→1.40 / Sharpe 1.68→**1.78** / Calmar 5.02→5.40 / MDD% 18.8→17.9 / PnL +$942; SPY 365d Sharpe 3.02→3.14 / Calmar 9.70→10.06 / PnL +$474; QQQ 365d Sharpe 1.32→**1.50** / PF 1.24→1.29 / PnL +$843. Per-symbol-arb sniff passes (both symbols win every metric); walk-forward sniff passes (365d directions match 730d on every metric). Only red cell: QQQ 365d MDD% widens 36.4→38.0 — accompanied by Sharpe +0.18 and PF +0.05, so variance grew less than return did. **Significance:** the first promotion sourced from a *new data dimension* (macro calendar) rather than from re-slicing OHLCV/indicator features — opens calendar/event/exogenous-data as a valid frontier after 5 consecutive entry-feature-diagnostic failures. `FOMC_DATES` constant mirrored across `replay_sweet_spot.py`, `replay_sweet_spot_qqq.py`, and `run_sweet_spot_agent.py` (live); needs updating when Fed publishes 2026-H2 meeting schedule. Live-replay parity to be verified next session via `verify_live_vs_replay --all-bars`. Disable with `--allow-fomc-days`.
>
...
| **FOMC sit-out** | **ON** (skip rate-decision days) | Skip the 8/year FOMC meeting end-dates (e.g. 2026-04-29, 2026-06-17). Promoted **2026-06-04** after macro-calendar diagnostic showed FOMC days have SPY stop-rate **53.8%** vs 18.7% baseline and net −$10.17 over 9 days; QQQ 31.0% vs 22.7% and −$9.42 over 11 days — the first feature in 2y to clear "elevated stop% AND negative aggregate P&L" on both symbols. A/B clean sweep both symbols both windows: SPY 730d Sharpe 3.02→3.14, Calmar 16.58→17.15, PnL +$1,017; QQQ 730d Sharpe 1.68→1.78, Calmar 5.02→5.40, PnL +$942; SPY 365d Sharpe +0.12, PnL +$474; QQQ 365d Sharpe **1.32→1.50**, PnL +$843. First **new-data-source** win — calendar/event features are a valid frontier. `FOMC_DATES` constant hardcoded through 2026-04-29 — extend as Fed publishes 2026-H2 schedule. Disable with `--allow-fomc-days`. |
| **Failed-bounce sit-out** | **ON** (gap ∈ (0, 0.50]%, prior_close_pos ≤ 0.20) | Skip days where today gaps up small (0 to +0.50%) AND yesterday's close sat in the bottom 20% of yesterday's range — a "failed bounce" archetype where bearish capitulation close + weak overnight bid lets the rally fail as bearish flow returns. Promoted **2026-06-06** after a 2D day-archetype scan revealed the cross-symbol-consistent negative cell (SPY 19 days mean −0.16R; QQQ 10 days mean −0.33R) that single-feature scans had missed. **A/B clean sweep all 8 cells under real-options pricing** (synth A/B had agreed in direction but overstated magnitudes — see [feedback_real_options_ab_discipline](../.claude/projects/c--Users-krish-options-algo-trader/memory/feedback_real_options_ab_discipline.md)): SPY 730d PF 1.84→2.00, Sharpe 3.12→**3.45**, Calmar 17.02→18.41, MDD% 5.7→5.3, PnL +$25; QQQ 730d Sharpe 1.81→1.99, MDD% 18.1→**16.7**, PnL +$18; SPY 365d Sharpe 3.15→**3.64**, MDD% 9.4→8.5, PnL +$17; QQQ 365d Sharpe 1.52→**1.84**, MDD% 37.7→36.3, PnL +$13. Sharpe lifts are *larger* on the 365d walk-forward than 730d for both symbols — strongest possible generalization sign. Second day-start filter to win (after FOMC); confirms calendar/regime features are the open frontier after entry-feature space exhausted. Disable with `--allow-failed-bounce`; tune via `--failed-bounce-gap-max` / `--failed-bounce-close-pos-max`. |
