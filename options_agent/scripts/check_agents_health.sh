#!/bin/bash
# Ping Discord if any expected agent container is down/unhealthy.
# Pipeline-wide notification: runs on the host via cron during market hours.
# Dedups via a state file so it alerts on transitions only (down / recovered),
# not on every run. Does NOT restart anything — ensure_agents.sh handles healing.
...
# quietly (ensure_agents.sh owns bringing Colima/Docker back up).
