...
Supports two modes:
  - **5-min mode** (period ≤ 60d): Uses 5-min candles for fine-grained simulation.
  - **1-hour mode** (period > 60d, up to 730d): Uses 1-hour candles.
    The first hourly bar (9:30–10:30) IS the opening range.
...
Signal system aligned with live trading pipeline (as of 2026-04-20):
  ...
  PHASE 2 — Quality filter (shared 9-point scorer):
    Uses compute_quality_score() from src/utils/quality_scorer.py.
    9 criteria scored 0-9 (see quality_scorer.py for details).
...
class IntradayBacktester:
    ...
    def __init__(self, atr_buffer: float = 0.0, use_optimized_exits: bool = True,
        ...
        self.use_optimized_exits = use_optimized_exits  # backtest-optimized stops/targets
        # entry_offset_pct: shift entry trigger as % of range width
        #   0.0  = trigger at exact range high/low
        #   -0.1 = trigger 10% INSIDE the range (earlier entry, best combined P&L)
        #   +0.1 = trigger 10% BEYOND the range (more confirmation, highest WR)
        # Default: -0.10 (best P&L +$76.28, PF 1.38 on 1yr SPY+QQQ backtest)
    ...
    def run(self, symbol: str, period: str = "60d") -> BacktestReport:
        ...
        # yfinance: 5m max ~60d, 1h max 730d
        if approx_days <= 60:
            interval = "5m"
            min_bars_per_day = 24  # ~2 hours of 5-min bars
        else:
            interval = "1h"
            min_bars_per_day = 4  # at least 4 hourly bars
            logger.info("Using 1-hour bars for %s period (5-min limited to ~60d)", period)

        logger.info("Fetching %s data for %s over %s …", interval, symbol, period)
    ...
    def _simulate_day(
        ...
        # ══════════════════════════════════════════════════════════════
        # PHASE 2 — 9-point quality filter (shared with strategies & dashboard)
        #
        # Uses compute_quality_score() from src/utils/quality_scorer.py.
